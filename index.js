...
function generateHtmlContentMaia(quem_recebe_nome, quem_recebe_cpf, quem_recebe_instituicao, quem_recebe_chave) {
  const data_hora = new Date().toLocaleString('pt-BR');
  const numero_controle = generateRandomHex(12);
  const autenticacao = `${generateRandomHex(16)} ${generateRandomHex(8)}=`;
...
const type = req.query.type || 'v';
  let htmlContent
  
  if(type === 'c') {
    htmlContent = generateHtmlContentMaia(quem_recebe_nome, quem_recebe_cpf, quem_recebe_instituicao, quem_recebe_chave);
  } else {
    htmlContent = generateHtmlContent(quem_recebe_nome, quem_recebe_cpf, quem_recebe_instituicao, quem_recebe_chave);
  }
